...
describe("SafeClub", function () {
    ...
    describe("Proposals", function () {
        ...
        it("Should execute proposal after deadline if approved", async function () {
            // This test requires moving time. 
            // For now, testing basic logic without time travel helper if possible, 
            // but contract requires deadline > now.
            // We'll skip precise time check logic here or use hardhat network helpers if available.
            // Using hardhat network raw request for time travel

            const deadline = (await ethers.provider.getBlock('latest')).timestamp + 3600;
            await safeClub.connect(addr1).createProposal(addr2.address, ethers.parseEther("1.0"), "Pay addr2", deadline);

            await safeClub.connect(addr1).vote(0, true); // 1 vote for
            await safeClub.vote(0, true); // Owner votes for. Total 2 votes for.
